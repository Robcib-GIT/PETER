...
Kp = 60;   % select between 30-100
max_iter = 20;
err_h_tol = 1;
err_pos_norm_tol = 0.7;
total_duration=0;

% for circumstances that needs at least one of the leg heights higher
% than 78mm or lower than 57mm , PETER is not able to do that.   
desired_x = 0;
desired_y = 0;
desired_h = 63; 

% Inıtial Safety
write_one_valve_millis(ser, 1, -2000);
write_one_valve_millis(ser, 2, -2000);
write_one_valve_millis(ser, 3, -2000);
...
try
    ...
    % calculation of desired p,r,h values w.r.t desired x,y,h values
    [desired_pitch, desired_roll, desired_hh] = xyh_to_prh(desired_x, desired_y, desired_h);

    % calculation of desired leg heights w.r.t desired p,r,h values
    ...
    while true
        ...
        if abs(pos_norm) < err_pos_norm_tol && abs(h_error) < err_h_tol
            ...
            for k = 1:max_iter
                ...
                pause_duration = max([200, abs(tA), abs(tB), abs(tC)]) / 1000;
